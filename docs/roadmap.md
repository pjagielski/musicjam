...
| Step | What | Effort |
| --- | --- | --- |
| 12.1 | ✓ **Done.** ★ **A loop track the engine plays in time.** `LoopTrack` names the audio and how many bars it fills; `LoopVoice` reads it with a fractional index at the rate that makes those bars the jam's bars, asked for every frame so a tempo change re-times the pass under way. A short loop starts again every so many bars, and a new pass ends the one still playing. Its gain and mute are live, like any track's. Faster is higher: 12.4 is what it would take not to be. | M |
