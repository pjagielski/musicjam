...
/**
 * Everything {@link NovasawVoice} needs to know to make a sound: what the patch's constructor used
 * to bake into its fields, now a value that can be handed around — stored in a patch, read frame by
 * frame by a voice, or replaced while a note is sounding when a knob moves.
 *
...
public record SynthParams(
        ...
        float detuneCents, float vibratoCents, float motionRateHz, float motion,
    ...
    public static SynthParams of(
        ...
        return new SynthParams(attackSeconds, decaySeconds, sustainLevel, releaseSeconds,
                detuneCents, vibratoCents, motionRateHz, motion,
    ...
    public SynthParams withCutoff(float hz) {
        return of(attackSeconds, decaySeconds, sustainLevel, releaseSeconds, detuneCents, vibratoCents,
                motionRateHz, motion, hz, resonance, filterEnvAmountHz, keyTrackHzPerSemitone,
                drive, outputTrim);
    }

    public SynthParams withResonance(float amount) {
        return of(attackSeconds, decaySeconds, sustainLevel, releaseSeconds, detuneCents, vibratoCents,
                motionRateHz, motion, cutoffHz, amount, filterEnvAmountHz, keyTrackHzPerSemitone,
                drive, outputTrim);
    }

    public SynthParams withEnvelope(float attack, float decay, float sustain, float release) {
        return of(attack, decay, sustain, release, detuneCents, vibratoCents, motionRateHz, motion,
                cutoffHz, resonance, filterEnvAmountHz, keyTrackHzPerSemitone, drive, outputTrim);
    }
}
